...
export default function UserProfile({ stats, currentUser, onSubmitFeedback, refreshData }) {
    ...
    return (
      <div className="glass animate-fade-in" style={{ padding: '40px 20px', textAlign: 'center', maxWidth: '600px', margin: '20px auto' }}>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
          Оберіть користувача у верхньому симуляторі для перегляду статистики
        </p>
      </div>
    );
  }
  ...
  const handleCommentChange = (regId, comment) => {
    setFeedbackState(prev => ({
      ...prev,
      [regId]: {
        rating: 5, // default
  ...
  const handleSendFeedback = async (regId) => {
    const data = feedbackState[regId] || { rating: 5, comment: '' };
    setLoading(prev => ({ ...prev, [regId]: true }));
    try {
      await onSubmitFeedback(regId, data.rating, data.comment);
      setSuccessMsg(prev => ({ ...prev, [regId]: 'Дякуємо! Ваш відгук збережено!' }));
      
      // Auto clear message and refresh stats
      setTimeout(async () => {
        await refreshData();
      }, 1500);
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: '700px', margin: '0 auto', padding: '16px 8px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {/* USER HEADER BANNER */}
      <div className="glass" style={{ padding: '24px', display: 'flex', alignItems: 'center', gap: '16px', border: '1px solid var(--border-glass)' }}>
        <div style={{
          width: '60px',
          height: '60px',
          borderRadius: '50%',
